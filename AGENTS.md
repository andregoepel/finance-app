...
rules → Codex API fallback with review queue. Host app composing
...
- `AndreGoepel.FinanceApp.Categorization` — rules engine + Codex API client
...
- **Codex categorization:** batch ~50 transactions per request, structured
...
- Write all issue and pull-request titles and descriptions in English.
- Write issue and pull-request descriptions as readable GitHub-flavored
  Markdown with real line breaks, blank lines between sections, and proper
  bullet lists. Never pass escaped `\n` sequences as body text.
- Prefer `--body-file` or another method that preserves multiline Markdown;
  verify the rendered body after creating or editing an issue or pull request.
